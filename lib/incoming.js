...
var swapSymlinks = require('./swap-symlink');
var Q       = require('q');

function processIncoming(deferred, args) {
    ...
    unpack(_args)
        .on('end', function () {
             ...
             * url etc etc
             */
            deferred.resolve({basename: args.basename, subdomain: args.subdomain, args: args});
        })
...
function getExistingRelease (args, outpath) {
     ...
     */
    var exists = fs.existsSync(path.join(conf.output, args.subdomain));
    var dirs   = [];
    var match  = [];

    if (exists) {
        dirs = fs.readdirSync(path.join(conf.output, args.subdomain));
    }
...
module.exports = function (incoming) {
    ...
    var basename    = releaseDir({
        timestamp: timestamp
    });

    var tasks = [processIncoming, swapSymlinks];
