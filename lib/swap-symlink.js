...
module.exports = function (flags) {
    ...
    return function (args) {
         ...
         */
        args.symlinks = {
            src: path.resolve(conf.output, args.subdomain, args.basename, conf.publicDir),
            target: path.resolve(conf.output, args.subdomain, conf.current)
        };
         ...
         */
        var symlinkcmd = symlinkCmd({basename: args.basename});
         ...
         */
        var opts = {
            cwd: path.resolve(conf.output, args.subdomain)
        };
...
function symlinkCmd (opts) {
    return ['ln -fns', opts.basename + '/' + conf.publicDir, conf.current].join(' ');
}
