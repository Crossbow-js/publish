...
module.exports = function (deferred, item) {
    ...
    logger.debug('sending release {cyan:%s', item.stream.filepath);

    form.append('user', item.config.get('user'));
    form.append('flags', JSON.stringify(item.config));
    form.append('subdomain', item.config.get('subdomain'));
    form.append('release', fs.createReadStream(item.stream.filepath));
